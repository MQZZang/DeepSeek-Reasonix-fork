...
func TestApprovedPlanAutoApproveEndsWithExecutionTurn(t *testing.T) {
	...
	runner.scripts = append(runner.scripts,
		func(input string) {
			exec.Session().Add(provider.Message{Role: provider.RoleAssistant, Content: "Plan:\n1. Create the file\n2. Update the file", ToolCalls: []provider.ToolCall{{
				ID: "sp1", Name: "submit_plan", Arguments: `{"title":"Plan","phases":[{"name":"Create the file"},{"name":"Update the file"}]}`,
			}}})
		},
		func(input string) {
			if input != c.ComposeSynthetic(planApprovedMessage) {
				t.Fatalf("approved execution input = %q, want %q", input, c.ComposeSynthetic(planApprovedMessage))
			}
