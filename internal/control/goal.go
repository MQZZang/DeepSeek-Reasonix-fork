...
type goalAdvanceInput struct {
	...
	markerOk   bool   // whether the reply ended with a recognized [goal:*] marker
...
func (g *goalMachine) markerlessCount() int {
	g.mu.Lock()
	defer g.mu.Unlock()
	return g.markerless
}
...
func (g *goalMachine) set(goal string, mode GoalResearchMode, todos []evidence.TodoItem) (string, []byte, bool) {
	...
	g.selfCheckDone, g.idleTurns, g.strict, g.markerless = false, 0, false, 0
	if goal == "" {
...
func (g *goalMachine) pause(todos []evidence.TodoItem) (changed bool, path string, data []byte, ok bool) {
	g.mu.Lock()
	defer g.mu.Unlock()
	if strings.TrimSpace(g.goal) == "" || g.status != GoalStatusRunning {
	...
	return true, path, data, ok
}

func (g *goalMachine) resume(todos []evidence.TodoItem) (changed bool, path string, data []byte, ok bool) {
	g.mu.Lock()
	defer g.mu.Unlock()
	if strings.TrimSpace(g.goal) == "" || (g.status != GoalStatusPaused && g.status != GoalStatusBlocked) {
		return false, "", nil, false
	}
	g.status = GoalStatusRunning
	g.blocks, g.block, g.idleTurns, g.markerless = 0, "", 0, 0
	path, data, ok = g.buildStateLocked(todos)
	return true, path, data, ok
}

func (g *goalMachine) restoreFromBranch(state agent.GoalState, status string, todos []evidence.TodoItem) (path string, data []byte, ok bool) {
	g.mu.Lock()
	defer g.mu.Unlock()
	g.goal = state.Text
	g.status = status
...
func (g *goalMachine) advance(in goalAdvanceInput) goalAdvanceResult {
	...
	// Markerless audit: tolerate one reply without a [goal:*] marker; two
	// consecutive pauses the goal so turns aren't burned silently.
	if notice == "" && in.status != GoalStatusComplete && in.status != GoalStatusBlocked {
		if in.markerOk {
			g.markerless = 0
		} else {
			g.markerless++
...
func (c *Controller) persistGoalState(path string, data []byte, ok bool) {
	...
	c.syncBranchMetaGoal()
}

func (c *Controller) syncBranchMetaGoal() {
	c.mu.Lock()
	path := c.sessionPath
	c.mu.Unlock()
	if path == "" {
	...
	goal, status, _ := c.goals.snapshot()
	goal = strings.TrimSpace(goal)
	keep := goal != "" && (status == GoalStatusRunning || status == GoalStatusPaused || status == GoalStatusBlocked)
	if !keep {
	...
	m.Goal = &agent.GoalState{
		Text:       goal,
		Status:     status,
		Markerless: c.goals.markerlessCount(),
	}
	if err := agent.SaveBranchMetaPreserveUpdated(path, m); err != nil {
		slog.Warn("controller: persist goal", "err", err)
	}
...
func (c *Controller) restoreGoalFromMeta(path string) {
	...
	c.goals.restoreFromBranch(*state, status, c.goalTodos())
	c.syncBranchMetaGoal()
	if status == GoalStatusPaused {
		c.notice(fmt.Sprintf(i18n.M.GoalRestoredFmt, ShortGoalForNotice(state.Text)))
	}
}
