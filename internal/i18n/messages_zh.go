...
var Chinese = Messages{
	...
	ChatStatusCycleHint:         "shift+tab 循环计划/问答 · ctrl+y yolo",
	ChatStatusCacheNowFmt:       "本次命中 %s",
	ChatStatusCacheAvgFmt:       "平均 %s",
	ChatStatusPlanApproval:      "Enter/y 批准并执行 · e 提修改意见 · n/Esc 继续规划 · PgUp/PgDn/Ctrl+Home/End 滚动",
	PlanApprovalPrompt:          "计划已生成（见上方）— Enter/y 批准执行,e 提修改意见,n/Esc 继续规划",
	PlanFeedbackPrompt:          "请描述对计划的修改意见 — Enter 发回模型修订,Esc 返回",
	PlanFeedbackSentFmt:         "↳ 已要求修订: %s",
	...
	CmdModel:        "切换模型",
	CmdMemory:       "查看记忆文件",
	CmdGoal:         "设置、暂停/恢复或清除当前目标",
	CmdAsk:          "问答模式：只回答、不改仓库（/ask off 退出）",
	CmdRemember:     "保存一条记忆",
	CmdForget:       "归档一条已存记忆",
	CmdMcp:          "MCP 服务器",
	CmdHooks:        "管理 hooks",
	...
	GoalCurrentFmt:         "目标：%s",
	GoalCurrentStatusFmt:   "目标（%s）：%s",
	GoalSetFmt:             "目标已设置 → %s",
	GoalCleared:            "目标已清除",
	GoalPaused:             "目标已暂停 — /goal resume 继续",
	GoalNothingToPause:     "目标：没有进行中的目标可暂停",
	GoalNothingToResume:    "目标：没有已暂停的目标可恢复",
	GoalResumedFmt:         "目标已恢复 → %s",
	GoalPausedMarkerless:   "目标已暂停：连续两轮回复缺少 [goal:*] 状态标记 — /goal resume 继续",
	GoalRestoredFmt:        "已恢复目标（暂停中）：%s — /goal resume 继续",
	AskModeOn:              "问答模式已开启 — 只回答、不改动仓库；/ask off 退出",
	AskModeOff:             "问答模式已关闭",
	PlanArtifactSavedFmt:   "计划已保存到 %s",
	PlanNudgeDeclined:      "该回复未作为计划提交——让模型提交计划即可唤起审批门",
